...
import { colorDark as c, pipeline } from "@xentral/config";
...
function Badge({ label, bg, fg }: { label: string; bg: string; fg: string }) {
  return <span style={{ fontSize: 11, fontWeight: 600, background: bg, color: fg, borderRadius: 999, padding: "2px 10px" }}>{label}</span>;
}
function Stage({ s }: { s: keyof typeof pipeline }) {
  const t = pipeline[s];
...
export default function ThemePreviewPage() {
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: c.surface.page, color: c.ink.DEFAULT, fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, sans-serif" }}>
      {/* Sidebar */}
      <aside style={{ width: 230, flexShrink: 0, background: c.shell.bar, borderRight: `1px solid ${c.line.DEFAULT}`, padding: "16px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "0 18px 16px" }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 700 }}>Xentral</span>
          <span style={{ fontSize: 10, fontWeight: 700, color: c.brand.primary, background: c.brand.primaryTint, borderRadius: 5, padding: "1px 6px" }}>NIGHT</span>
        </div>
        {NAV.map((sec) => (
          <div key={sec.g} style={{ padding: "8px 0" }}>
            <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: c.ink.soft, padding: "0 18px 6px" }}>{sec.g}</div>
            {sec.items.map((it, i) => {
              const on = sec.g === "Revenue" && i === 1;
              return <div key={it} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 18px", fontSize: 13.5, color: on ? c.brand.primary : c.ink.mid, background: on ? c.brand.primaryTint : "transparent", fontWeight: on ? 600 : 400 }}><span style={{ width: 7, height: 7, borderRadius: 2, background: on ? c.brand.primary : c.line.strong }} />{it}</div>;
            })}
          </div>
        ))}
      </aside>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <header style={{ height: 56, flexShrink: 0, background: c.shell.alt, borderBottom: `1px solid ${c.line.DEFAULT}`, display: "flex", alignItems: "center", gap: 12, padding: "0 22px" }}>
          <div style={{ flex: 1, maxWidth: 420, height: 34, background: c.surface.sunken, border: `1px solid ${c.line.DEFAULT}`, borderRadius: 8, display: "flex", alignItems: "center", padding: "0 12px", color: c.ink.soft, fontSize: 13 }}>Search leads, contacts, deals…</div>
          <span style={{ fontSize: 12, fontWeight: 600, color: c.brand.primary, background: c.brand.primaryTint, borderRadius: 7, padding: "5px 10px" }}>✦ Ask AI</span>
        ...
        <main style={{ flex: 1, padding: "22px 24px", overflowY: "auto" }}>
          <div style={{ fontSize: 20, fontWeight: 600, marginBottom: 2 }}>Good evening, Nami</div>
          <div style={{ fontSize: 13, color: c.ink.mid, marginBottom: 18 }}>Dark Night theme preview — SAP Fiori Horizon palette</div>
          ...
          <section style={{ background: c.surface.card, border: `1px solid ${c.line.DEFAULT}`, borderRadius: 10, padding: "16px 18px", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              <h2 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>Deals</h2>
              <span style={{ fontSize: 12, fontWeight: 600, background: c.brand.primary, color: c.ink.onPrimary, borderRadius: 8, padding: "6px 12px" }}>+ New deal</span>
            </div>
            <div>
              ...
              {ROWS.map((r) => (
                <div key={r.name} style={{ display: "flex", alignItems: "center", padding: "11px 0", borderBottom: `1px solid ${c.line.DEFAULT}`, fontSize: 13.5 }}>
                  ...
                  <span style={{ width: 110, textAlign: "right", fontWeight: 600 }}>{aed(r.val)}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            <section style={{ flex: 1, minWidth: 280, background: c.surface.card, border: `1px solid ${c.line.DEFAULT}`, borderRadius: 10, padding: "16px 18px" }}>
              <h2 style={{ fontSize: 14, fontWeight: 600, margin: "0 0 12px" }}>Status badges</h2>
            ...
            <section style={{ flex: 1, minWidth: 280, background: c.brand.primaryTint, border: `1px solid ${c.brand.primary}55`, borderRadius: 10, padding: "16px 18px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 8 }}>
                <span style={{ width: 26, height: 26, borderRadius: 8, background: c.brand.primary, color: c.ink.onPrimary, display: "flex", alignItems: "center", justifyContent: "center" }}>✦</span>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, background: c.brand.primary, color: c.ink.onPrimary, borderRadius: 8, padding: "7px 13px" }}>Send reminder</span>
          ...
          <p style={{ fontSize: 11, color: c.ink.soft, textAlign: "center", marginTop: 24 }}>Dark Night preview · colorDark + pipeline tokens from @xentral/config · standalone (global default stays light)</p>
        </main>
      </div>
    </div>
  );

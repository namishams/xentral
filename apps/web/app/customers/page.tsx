...
import { AppShell, PageTitleRow, ExportMenu, ImportButton, KPICard, Button, DataTable, Panel, PanelHeader, PanelBody, EmptyState, type Column } from "@xentral/ui";
  ...
  return (
    <AppShell active="customers">
      <PageTitleRow title="Customers" subtitle={`${rows.length} billing customer${rows.length === 1 ? "" : "s"}${totalOutstanding > 0 ? ` · ${aed(totalOutstanding, cur)} outstanding` : ""}`}
        actions={<div style={{ display: "flex", gap: 8 }}><ExportMenu entity="customers" /><ImportButton entity="customers" /><Button onClick={() => { window.location.href = "/quotations/new"; }}>New quote</Button><Button variant="primary" onClick={() => { window.location.href = "/customers/new"; }}>+ New customer</Button></div>} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 300px", gap: 16, alignItems: "start" }}>
        ...
        <Panel>
          ...
          <PanelBody flush>
            {debtors.length === 0 ? <div style={{ padding: 16, textAlign: "center", fontSize: 13, color: color.ink.soft }}>No outstanding balances. 🎉</div>
              : debtors.map((c) => (
                <a key={c.id} href={`/customers/${c.id}`} className="xui-row-link" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: `1px solid ${color.line.DEFAULT}`, textDecoration: "none", color: color.ink.DEFAULT }}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 9, minWidth: 0 }}><span style={{ width: 24, height: 24, flexShrink: 0, borderRadius: 7, background: color.surface.sunken, color: color.ink.mid, fontSize: 10, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{initials(c.name)}</span><span style={{ minWidth: 0 }}><span style={{ display: "block", fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{c.name}</span><span style={{ fontSize: 12, color: color.ink.soft }}>{c.invoiceCount} invoice{c.invoiceCount === 1 ? "" : "s"}</span></span></span>

...
export default function SalesTeamsPage() {
  ...
  const memberRow = (u: User, t?: Team) => (
    <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", borderBottom: `1px solid ${color.line.DEFAULT}` }}>
      <span style={{ width: 26, height: 26, flexShrink: 0, borderRadius: "50%", background: color.brand.primaryTint, color: color.brand.primary, fontSize: 11, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{initials(u.name || "?")}</span>
      ...
      <select value={u.salesRole || ""} onChange={(e) => setRole(u.id, e.target.value)} disabled={busy} style={{ height: 28, fontSize: 12, border: `1px solid ${color.line.strong}`, borderRadius: 7, background: color.surface.card, color: roleTone(u.salesRole), fontWeight: 600, padding: "0 6px" }}>
      ...
      {loading ? <div style={{ padding: 40, textAlign: "center", color: color.ink.soft }}>Loading…</div> : (
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 300px", gap: 16, alignItems: "start" }}>
              ...
              : teams.map((t) => {
                ...
                return (
                  <Panel key={t.id}>
                    ...
                    <PanelBody flush>
                      {mem.length === 0 ? <div style={{ padding: 14, textAlign: "center", fontSize: 13, color: color.ink.soft }}>No members yet.</div> : mem.map((u) => memberRow(u, t))}
                      {addTo === t.id ? (
                        <div style={{ padding: "10px 14px", background: color.surface.page, display: "flex", gap: 8, alignItems: "center" }}>
                          <select autoFocus onChange={(e) => { if (e.target.value) { assign(e.target.value, t.id); setAddTo(null); } }} style={{ flex: 1, height: 32, fontSize: 13, border: `1px solid ${color.line.strong}`, borderRadius: 8, background: color.surface.card, color: color.ink.DEFAULT, padding: "0 8px" }}>
          ...
          <Panel>
            ...
            <PanelBody flush>
              {unassigned.length === 0 ? <div style={{ padding: 16, textAlign: "center", fontSize: 13, color: color.ink.soft }}>Everyone is on a team. 🎉</div>
                : unassigned.map((u) => (
                  <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", borderBottom: `1px solid ${color.line.DEFAULT}` }}>
                    <span style={{ width: 26, height: 26, flexShrink: 0, borderRadius: "50%", background: color.surface.sunken, color: color.ink.mid, fontSize: 11, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{initials(u.name || "?")}</span>
                    ...
                    {teams.length > 0 ? (
                      <select onChange={(e) => { if (e.target.value) assign(u.id, e.target.value); }} disabled={busy} style={{ height: 28, fontSize: 12, border: `1px solid ${color.line.strong}`, borderRadius: 7, background: color.surface.card, color: color.brand.primary, fontWeight: 600, padding: "0 6px" }}>

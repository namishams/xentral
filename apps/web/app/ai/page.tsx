  ...
  return (
    <AppShell active="ai" fullBleed>
      <div style={{ height: "100%", display: "flex", flexDirection: "column", minHeight: 0, background: color.surface.page }}>
        <div style={{ flexShrink: 0, padding: "18px 24px", borderBottom: `1px solid ${color.line.DEFAULT}`, background: "#fff" }}>
            ...
            {turns.map((t, i) => (
              <div key={i} style={{ display: "flex", justifyContent: t.who === "you" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "78%" }}>
                  ...
                  {t.action ? (
                    <div style={{ marginTop: 8, background: color.brand.primaryTint, border: `1px solid ${color.brand.primary}22`, borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ flexShrink: 0, background: "#fff", borderTop: `1px solid ${color.line.DEFAULT}`, padding: "12px 24px" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
              {SUGGESTIONS.map((s) => <button key={s} onClick={() => setQ(s)} style={{ fontSize: 12.5, background: "#fff", border: `1px solid ${color.line.DEFAULT}`, borderRadius: 999, padding: "6px 12px", color: color.ink.DEFAULT, cursor: "pointer" }}>{s}</button>)}
            </div>

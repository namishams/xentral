...
export default function QuotationsPage() {
  ...
  const acceptedValue = ALL.filter((r) => r.status === "accepted").reduce((s, r) => s + r.total, 0);
  const acc = ALL.filter((r) => r.status === "accepted").length;
  const closed = ALL.filter((r) => r.status === "rejected" || r.status === "expired").length;
  ...
  return (
    <AppShell active="quotations">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 16 }}>
        ...
        <KPICard label="Accepted" value={aedShort(acceptedValue)} note={`${acc} won`} noteTone={color.status.positive} />
        ...
        <KPICard label="Avg quote" value={aedShort(avg)} note="per document" noteTone={color.ink.soft} />
        <KPICard label="Customers" value={String(new Set(ALL.map((r) => r.customer)).size)} note="quoted" noteTone={color.ink.soft} />
      </div>
      ...
      {visibleGroups.length === 0 ? (
        <EmptyState title="No quotes match your filters" hint="Try a different quote, customer or status." action={<Button variant="primary" onClick={() => { setQ(""); setFilt("all"); }}>Clear filters</Button>} />
      ) : (

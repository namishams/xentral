...
const COLUMNS: Column<CompanyRow>[] = [
  { key: "name", header: "Company", render: (r) => (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 10, minWidth: 0 }}>
      ...
      <span style={{ minWidth: 0 }}>
        ...
        <span style={{ display: "block", fontSize: 12, color: color.ink.soft, lineHeight: "15px" }}>{r.segment || "No segment"}</span>
      </span>
    </span>
  ) },
  ...
  { key: "city", header: "Location", width: 170, render: (r) => (
    <span style={{ minWidth: 0 }}>
      ...
      {r.country ? <span style={{ display: "block", fontSize: 12, color: color.ink.soft, lineHeight: "14px" }}>{r.country}</span> : null}
    </span>
  ) },
...
function Card({ r }: { r: CompanyRow }) {
  return (
    <a href={`/companies/${r.id}`} style={{ textDecoration: "none", background: color.surface.card, border: `1px solid ${color.line.DEFAULT}`, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 10, boxShadow: "0 1px 2px rgba(16,24,40,0.04)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13, color: color.ink.mid }}>

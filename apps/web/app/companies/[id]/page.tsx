          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 18px", marginTop: 9 }}>
              {([["Owner", a.ownerName || "Unassigned"], ["Segment", a.segment || "—"], ["Open pipeline", openValue > 0 ? aed(openValue) : "—"], ["Last touch", d.activities[0]?.at?.split(",")[0] ?? "—"]] as [string, string][]).map(([l, v]) => (
                <div key={l} style={{ minWidth: 0 }}><div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: 0.4, textTransform: "uppercase", color: color.ink.soft }}>{l}</div><div style={{ fontSize: 12.5, fontWeight: 600, color: color.ink.DEFAULT, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 220 }}>{v}</div></div>
              ))}
            </div>
          </div>
        </div>

...
const CFG = {
  invoice: { active: "invoice", title: "New invoice", api: "/api/books/invoices", list: "/invoices", dateLabel: "Due date", dateKey: "dueDate" },
  quote: { active: "quotations", title: "New quote", api: "/api/books/quotes", list: "/quotations", dateLabel: "Valid until", dateKey: "validUntil" },
} as const;
...
export function BooksBuilder({ kind }: { kind: Kind }) {
  const cfg = CFG[kind];
  ...
  async function save() {
    ...
    const payload: Record<string, unknown> = {
      currency, notes,
      [cfg.dateKey]: date || null,
      lines: lines.map((l) => ({ name: l.name, qty: parseFloat(l.qty) || 0, unitPrice: parseFloat(l.unitPrice) || 0, vatRate: parseFloat(l.vatRate) || 0, discountPct: parseFloat(l.discountPct) || 0 })),
    };
    ...
    try {
      ...
      if (res.ok) { window.location.href = cfg.list; }
  ...
  return (
    <AppShell active={cfg.active}>
      <PageTitleRow title={cfg.title} subtitle="Build a draft, then send or convert later"
        actions={<div style={{ display: "flex", gap: 8 }}><Button onClick={() => { window.location.href = cfg.list; }}>Cancel</Button><Button variant="primary" onClick={save} disabled={!canSave}>{saving ? "Saving…" : "Create draft"}</Button></div>} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0,2fr) minmax(0,1fr)", gap: 16 }}>
        ...
        <section style={{ background: color.surface.sunken, border: `1px solid ${color.line.DEFAULT}`, borderRadius: 10, padding: "16px 18px" }}>
          ...
          <div style={{ marginTop: 14 }}><Button variant="primary" onClick={save} disabled={!canSave}>{saving ? "Saving…" : "Create draft"}</Button></div>
        </section>
      </div>
      <p style={{ fontSize: 11, color: color.ink.soft, textAlign: "center", marginTop: 18 }}>{cfg.title} · saves a DRAFT · tenant-scoped</p>
    </AppShell>
  );
}

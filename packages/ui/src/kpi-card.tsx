...
export function KPICard({ label, value, note, noteTone }: {
  ...
  return (
    <div style={{ flex: 1, minWidth: uiConstants.card.kpi.minWidth, maxWidth: uiConstants.card.kpi.maxWidth, height: uiConstants.card.kpi.height, background: color.surface.card, border: `1px solid ${color.line.DEFAULT}`, borderRadius: 10, boxShadow: shadow.none, padding: "12px 16px", display: "flex", flexDirection: "column", justifyContent: "center", overflow: "hidden" }}>
      ...
      {note && <div style={{ fontSize: 12, color: noteTone ?? color.ink.soft, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{note}</div>}
    </div>
  );
}
